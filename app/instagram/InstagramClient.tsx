...
export const InstagramClient: React.FC = () => {
  ...
  const handleFetch = (url: string) => {
    const check = validateUrlForPlatform(url, 'instagram');
    if (!check.valid) {
      setDownloadError({
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 animate-fade-in">
      <div className="text-center space-y-3">
        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-pink-500/10 text-pink-500 font-bold text-xs">
          <PlatformIcon platform="instagram" className="w-4 h-4" /> Instagram Reel
        </div>
        ...
        <p className="text-sm sm:text-base text-slate-600 dark:text-zinc-400 max-w-xl mx-auto">
          Paste a public Reel link and choose your preferred quality.
        </p>
      </div>
