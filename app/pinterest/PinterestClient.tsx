...
export const PinterestClient: React.FC = () => {
  ...
  const handleFetch = (url: string) => {
    const check = validateUrlForPlatform(url, 'pinterest');
    if (!check.valid) {
      setDownloadError({
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 animate-fade-in">
      <div className="text-center space-y-3">
        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-red-500/10 text-red-500 font-bold text-xs">
          <PlatformIcon platform="pinterest" className="w-4 h-4" /> Pinterest Video Saver
        </div>
        ...
        <p className="text-sm sm:text-base text-slate-600 dark:text-zinc-400 max-w-xl mx-auto">
          Save Pinterest pin videos and Idea Pins in HD MP4 quality.
        </p>
      </div>
