...
export const VimeoClient: React.FC = () => {
  ...
  const handleFetch = (url: string) => {
    const check = validateUrlForPlatform(url, 'vimeo');
    if (!check.valid) {
      setDownloadError({
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 animate-fade-in">
      <div className="text-center space-y-3">
        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-cyan-500/10 text-cyan-500 font-bold text-xs">
          <PlatformIcon platform="vimeo" className="w-4 h-4" /> Vimeo Downloader
        </div>
        ...
        <p className="text-sm sm:text-base text-slate-600 dark:text-zinc-400 max-w-xl mx-auto">
          Download high quality Vimeo videos in 4K, 1080p, and 720p HD MP4 format.
        </p>
      </div>
